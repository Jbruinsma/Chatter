      ...
      <form class="form" @submit.prevent="handleSubmit">
        ...
        <div v-if="!isLogin" class="option" role="group" aria-labelledby="public-title">
          <div class="option-label">
            ...
            <div class="option-hint">{{ profileStatusMessage }}</div>
          </div>
...
const profileStatusMessage = computed(() => isPublic.value ? 'Anyone can follow you' : 'Followers will be managed by requests')
...
async function handleResponse(response){
  if (response.error){
    ...
    const notificationPreferences = response.notificationPreferences
    const loggedInId = response.id
    await setSuccessMessage(responseMessage)
    userStore.login(loggedInUsername, loggedInId)
    chatStore.login(loggedInId, notificationPreferences)
